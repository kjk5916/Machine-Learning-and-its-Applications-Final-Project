...
class BaseModel(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # self.encoder_old = self.base_model(zero_init_residual=zero_init_residual)
        # self.features_dim = self.encoder_old.inplanes
        # if cifar:
        #     self.encoder_old.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=2, bias=False)
        #     self.encoder_old.maxpool = nn.Identity()
        # self.encoder = nn.Sequential(*list(self.encoder_old.children())[:-1])

        # Note that the output size is batch_size x 512 x 5 x 5
