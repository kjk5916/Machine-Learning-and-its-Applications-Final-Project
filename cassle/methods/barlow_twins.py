...
class BarlowTwins(BaseModel):
    ...
    def forward(self, X, *args, **kwargs):
        ...
        # Apply global average pooling on the feature maps
        # Assuming out["feats"] contains the feature map of shape (batch_size, 512, H, W)
        out["feats"] = nn.AdaptiveAvgPool2d((1, 1))(out["feats"])
        
        # Flatten the pooled output to match the expected input of the projector
        out["feats"] = out["feats"].view(out["feats"].size(0), -1)  # Shape: (batch_size, 512)
    ...
    def training_step(self, batch: Sequence[Any], batch_idx: int) -> torch.Tensor:
        ...
        # Apply global average pooling on the feature maps
        # Assuming out["feats"] contains the feature map of shape (batch_size, 512, H, W)
        feats1 = nn.AdaptiveAvgPool2d((1, 1))(feats1)
        feats1 = feats1.view(feats1.size(0), -1)  # Shape: (batch_size, 512)

        feats2 = nn.AdaptiveAvgPool2d((1, 1))(feats2)
        feats2 = feats2.view(feats1.size(0), -1)  # Shape: (batch_size, 512)
